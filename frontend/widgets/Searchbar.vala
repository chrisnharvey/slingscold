...
namespace Slingshot.Frontend {
    ...
    public class Searchbar : Gtk.HBox {
        ...
        public Searchbar (string hint) {
            ...
            // Wrapper 
            var wrapper = new Gtk.HBox (false, 0); //espacio entre el icono de buscar y la frase "Empiese a ..."
            this.add (wrapper);
            
            // Pack gtk-find icon
            var search_icon_wrapper = new Gtk.EventBox ();
            search_icon_wrapper.set_visible_window (false);
            this.search_icon = new Gtk.Image.from_stock("gtk-find", Gtk.IconSize.MENU); //icono de buscar
